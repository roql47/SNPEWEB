...
export default function CompanyClass() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('기업·기관 프로그램 신청 저장 실패:', err)
  ...
  return (
    <>
      <PageBanner
        title={t('pages.companyClass')}
        subtitle={t('pages.companyClassSub')}
      />
      ...
      <section className="py-16 md:py-24">
        <div className="max-w-5xl mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
            {[
              { icon: Building2, title: '방문 특강', desc: '기업·기관 방문하여 진행하는 맞춤형 특강' },
              { icon: Users, title: '그룹 수업', desc: '10~50명 규모의 소그룹/대그룹 수업' },
              { icon: Clock, title: '정기 프로그램', desc: '주 1~2회 정기적 웰니스 프로그램' },
              ...
              <div key={i} className="bg-white border border-gray-100 rounded-2xl p-6 text-center hover:shadow-lg hover:-translate-y-1 transition-all">
                <div className="w-14 h-14 rounded-2xl bg-snpe-dark/10 text-snpe-dark flex items-center justify-center mx-auto mb-4">
                  <item.icon size={24} />
                </div>
          ...
          {/* 기업·기관 프로그램 신청하기 */}
          <div id="apply" className="bg-snpe/5 rounded-3xl p-6 md:p-10">
            <div className="text-center mb-8">
              <h3 className="text-2xl font-bold text-gray-900 mb-2">기업·기관 프로그램 신청하기</h3>
              <p className="text-sm text-gray-600">아래 항목을 작성해 주시면 담당자가 확인 후 연락드립니다.</p>
            </div>
            ...
            {submitted ? (
              <div className="max-w-lg mx-auto text-center py-8">
                ...
                <h4 className="text-xl font-bold text-gray-900 mb-3">신청이 접수되었습니다</h4>
                <p className="text-gray-600 mb-8">담당자가 확인 후 빠르게 연락드리겠습니다. 감사합니다.</p>
                <button
              ...
              <form onSubmit={handleSubmit} className="max-w-3xl mx-auto space-y-6">
                <div className="grid sm:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">기업/기관명 *</label>
                    <input
                      type="text" required value={form.organization} onChange={update('organization')}
                  ...
                  <div>
                    ...
                    <input
                      type="text" required value={form.manager} onChange={update('manager')}
                      placeholder="담당자 성함"
                      className="w-full h-11 px-4 rounded-lg border border-gray-200 text-sm focus:outline-none focus:border-snpe focus:ring-1 focus:ring-snpe"
                    />
                  </div>
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      type="text" value={form.preferredDate} onChange={update('preferredDate')}
                  ...
                  <div>
                    ...
                    <input
                      type="number" min="1" value={form.participants} onChange={update('participants')}
                      placeholder="예: 30"
                      className="w-full h-11 px-4 rounded-lg border border-gray-200 text-sm focus:outline-none focus:border-snpe focus:ring-1 focus:ring-snpe"
                    />
                  </div>
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  <div>
                    ...
                    <select
                      value={form.locationType} onChange={update('locationType')}
                  ...
                  <div>
                    ...
                    <select
                      value={form.requestType} onChange={update('requestType')}
                      className="w-full h-11 px-4 rounded-lg border border-gray-200 text-sm focus:outline-none focus:border-snpe focus:ring-1 focus:ring-snpe bg-white"
                    >
                      <option value="">선택하세요</option>
                      <option value="one-time">1회 특강</option>
                      <option value="workshop">워크숍</option>
                      <option value="regular">정기 수업</option>
                ...
                <div>
                  ...
                  <textarea
                    required value={form.message} onChange={update('message')} rows={6}
                ...
                <label className="flex items-start gap-2.5 text-sm text-gray-700">
                  <input
                    type="checkbox" required checked={form.agree} onChange={update('agree')}
                    className="mt-0.5 rounded border-gray-300 text-snpe-dark focus:ring-snpe-dark"
                  />
                  <span>
                    개인정보 수집 및 이용에 동의합니다. 수집된 정보는 기업·기관 프로그램 신청 상담 및 운영 목적에 한하여 사용됩니다.
                  </span>
                </label>
                ...
                >
                  <Send size={16} />
                  {submitting ? '신청 중...' : '기업·기관 프로그램 신청하기'}
                </button>
              </form>
            )}
          </div>
